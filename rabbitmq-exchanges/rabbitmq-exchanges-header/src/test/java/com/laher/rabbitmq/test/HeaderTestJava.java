...
public class HeaderTestJava {
    ...
    public void push() throws IOException, TimeoutException {
        ...
        // 声明交换机
        channel.exchangeDeclare("header_events", BuiltinExchangeType.HEADERS);
        ...
        channel.queueDeclare("headerTest", false, false, false, null);

        // 头部匹配
        Map<String, Object> arguments = new HashMap();
        // x-match为特殊的header，all匹配所有header，any匹配其中一个header
//        arguments.put("x-match", "all");
        arguments.put("x-match", "any");
        arguments.put("type", "json");
        arguments.put("format", "jpg");

        // 绑定关系
        channel.queueBind("headerTest", "header_events", "headerTest", arguments);

        // 发布消息（交换机名称，路由Key，强制性，持久化消息，发送消息）
        // 强制性：没有找到合适的队列，则调用basic.return将消息返回给生产者，若为false，则broker把消息丢弃
        channel.basicPublish("header_events", "headerTest", false, null, "hello rabbit header".getBytes("UTF-8"));
    ...
    public void pull() throws IOException, TimeoutException, InterruptedException {
        ...
        // 声明队列
        channel.queueDeclare("header_events", false, false, false, null);
        ...
        // 监听队列（队列名称，自动回复，消费方法）
        channel.basicConsume("header_events", true, consumer);
        //等待回调函数执行完毕之后 ， 关闭资源
